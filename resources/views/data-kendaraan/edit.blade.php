        ...
        <div class="col-md-8 mx-auto">
            <div class="card shadow-sm">
                <div class="card-header bg-warning text-dark">
                    <h5 class="mb-0"><i class="fas fa-edit"></i> Edit Kendaraan</h5>
                </div>
                <div class="card-body">
                    <form method="POST" 
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                   id="platInput"
                                   name="plat_nomor"
                                   value="{{ old('plat_nomor', $data_kendaraan->plat_nomor) }}"
                            ...
                            <small class="text-muted">
                                ...
                                Plat saat ini: <strong>{{ $data_kendaraan->plat_nomor }}</strong>
                            </small>
                        </div>
                            ...
                            <select name="id_pemilik" 
                                ...
                                @foreach($pemiliks as $p)
                                    <option value="{{ $p->id_pemilik }}"
                                        {{ old('id_pemilik', $data_kendaraan->id_pemilik) == $p->id_pemilik ? 'selected' : '' }}>
                                        {{ $p->nama }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="id_tipe" 
                                ...
                                @foreach($tipes as $t)
                                    <option value="{{ $t->id_tipe }}"
                                        {{ old('id_tipe', $data_kendaraan->id_tipe) == $t->id_tipe ? 'selected' : '' }}>
                                        {{ $t->tipe_kendaraan }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" 
                                    ...
                                    required>
                                <option value="aktif" {{ old('status', $data_kendaraan->status) == 'aktif' ? 'selected' : '' }}>
                                    Aktif
                                </option>
                                <option value="nonaktif" {{ old('status', $data_kendaraan->status) == 'nonaktif' ? 'selected' : '' }}>
                                    Nonaktif
                                </option>
                            </select>
                            @error('status')
...
(function() {
    ...
    console.log('=== KENDARAAN EDIT - AUTO FORMAT START ===');
    ...
    formKendaraan.addEventListener('submit', function(e) {
        ...
        btnSubmit.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengupdate...';
    ...
    console.log('=== KENDARAAN EDIT - AUTO FORMAT READY ===');
